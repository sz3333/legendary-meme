...
class AutoReactMod(loader.Module):
    strings = {
        ...
        "trigger_set": "✅ Установлено триггерное слово: {}"
    }
    ...
    def __init__(self):
        self.config = loader.ModuleConfig(
            ...
            "trigger_word",
            "",
            "Слово-триггер для автореакции (оставьте пустым для всех сообщений)"
        )
    ...
    @loader.command(ru_doc="Установить реакцию (эмодзи или ID для Premium)")
    ...
    @loader.command(ru_doc="Установить слово-триггер для автореакций")
    async def settrigger(self, message):
        args = utils.get_args_raw(message)
        self.config["trigger_word"] = args
        await utils.answer(message, self.strings["trigger_set"].format(args or "нет"))
    ...
    async def watcher(self, message):
        ...
        if trigger and trigger not in text.lower():
            return  # триггер установлен, но слово не найдено — выходим
